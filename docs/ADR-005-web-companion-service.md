...
## 修订：章节级并发翻译（2026-09-16）

本节覆盖第 6 节中“单任务状态机全局生效”的原决策；其余 Web 伴读边界不变。

- App 与 Web 伴读共用同一个进程级 `TranslationCoordinator`，但共享的是**章节任务集合**，不是全局唯一任务。任务以 `bookId + chapterId` 识别：不同章节可以并行，同一章节重复开始互斥且幂等。
- 阅读器当前显示的章节只决定 App 展示哪一个运行期状态，不拥有任务生命周期；切章、退出阅读器或由另一端启动其他章节，都不取消已在运行的章节任务。
- `translationRunId` 仍是章节级 stale 防护。重试只替换目标章节的代际，取消也只作用于目标章节；旧代际的完成、失败或取消写入不能覆盖该章当前代际。
- Web 端能力边界不扩大：仍只提供单章“开始/重试”，不提供取消，也不展示 SSE 流式译文。网页继续轮询 Room 中的章节耐久状态；准备、思考、流式文本等进程内实时状态不作为 Web API 契约。
- 伴读服务活跃时仍由其 WakeLock/WifiLock 覆盖翻译保活；伴读未开启时，独立翻译前台服务按全部活动网络任务的集合启停，而不是随任一单章任务结束即停止。

## 后果

- 新增依赖仅 NanoHTTPD 一个；权限（INTERNET/WAKE_LOCK/前台服务）与 cleartext 已具备。
- `TranslationCoordinator` 归属进程级组合根，App 与 Web 共用章节任务集合；章节级并发是产品行为，不再维持原“全局单任务”语义。
