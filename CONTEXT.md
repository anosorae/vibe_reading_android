...
| **书籍封面 (Book Cover)** | 书架展示的封面图：来自 EPUB 内置封面或用户上传的本地图片（用户上传优先）；TXT 或封面缺失时回退程序化渐变占位。用户上传的封面可移除，EPUB 有内置封面备份时可恢复原封面 | 格式徽标（已否决） |
...
| **段落跨页续排 (Paragraph Split Fill)** | 段落放不下页面剩余空间时按行边界切分填满当前页，剩余文本顶格（无首行缩进）续排到下一页；所有片段绑定同一原文范围 | 整段挪页、双语原子性（已废止，ADR-004） |
...
| **内置字体 (Built-in Font)** | 字体选择器内置的开源字库（OFL 协议），未下载时点击即从镜像源下载到本地；中英槽位按字形过滤 | 系统字体、自定义字体 |
...
| 正文插图与封面文件 | 插图链接语法 + `BookImageStore`（私有目录解析、内存位图缓存、删书清理、封面命名与降采样/EXIF 旋正的唯一落点） |
...
- **翻译门控**：英文书两种显示模式都翻译当前章（气泡与中文模式共用同一份中文译文），开启「提前翻译下一章」后空闲时自动预译下一章；中文书保持仅 en 模式翻译。未译回退对称：中文书 en 模式→中文原文、英文书 zh 模式→英文原文，均自动翻译自愈；英文书 en 模式下译文到达不触发重排。
...
- **删书清理图片**：删除书籍时同步删除其私有目录下的插图和封面文件。封面按 `{bookId}.` / `{bookId}_` 全名前缀匹配清理，覆盖老式 `{id}.jpg`、内嵌备份 `{id}_embedded.jpg`、用户封面 `{id}_{哈希}.jpg` 与残留 `.tmp`。
- **封面文件名承载来源与刷新**：`{id}_embedded.jpg` 是导入期内嵌封面且导入后不再改写，充当「恢复原封面」的备份；用户上传封面存为 `{id}_{内容哈希}.jpg`，靠路径变化触发书架 `BookCover`（以 path 为 key 的 `produceState`）重新解码——同名覆盖不会刷新。是否显示「恢复原封面」由 `BookImageStore.canRestoreEmbeddedCover` 判定（当前是用户封面且备份存在），UI 与 ViewModel 共用这一判据。
