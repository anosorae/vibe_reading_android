...
- Room schema 通过 KSP 输出到 `app/schemas`；当前数据库版本为 16，四个实体 `BookEntity`/`ChapterEntity`/`LlmProfileEntity`/`ReadingTimeEntity`，迁移链 `MIGRATION_2_3` … `MIGRATION_15_16` 全部手写注册（v6→v7 新增 `llm_profiles` 表，v7→v8 增 `temperature`/`topP`，v9→v10 books 增 `languageMode`，v11→v12 books 增 `format`/`coverPath` 支持 EPUB，v12→v13 books 增 `sourceLanguage` 支持英文原版书 ADR-003，v13→v14 llm_profiles 增 `maxOutputTokens`，v14→v15 移除上下文增强三列，v15→v16 新增 `reading_time_daily` 表支持阅读时长统计）。实体模型使用 `lastReadOffset`；`chapters.translationRunId` 提供翻译任务的数据库级 stale 防护；书架「已译章节数」由 chapters 表 DONE 状态实时派生，`books.translatedChapters` 冗余列已移除。

## 目录结构

- `app/src/main/java/com/vibereading/app/`
  - `data/` — Room 本地库：`local/entity`（`BookEntity`/`ChapterEntity`/`LlmProfileEntity`/`ReadingTimeEntity`）、`local/dao`（`BookDao`/`ChapterDao`/`LlmProfileDao`/`ReadingTimeDao`，含 `AppDatabase` 迁移链）、`remote/`（`TranslationService` 接口 + `LlmApiService` SSE 实现）、`repository/`（`BookRepository`/`ChapterRepository`/`LlmProfileRepository`/`ReadingTimeRepository`；`SettingsRepository` 是偏好组合根，按域拆为 `reading`/`theme`/`bookshelf`/`companion`/`llmLegacy` 五个 facet，各自一个 Store 类）、`dict/`（`DictDatabase` 内嵌词典只读访问）、`image/`（`BookImageStore`：EPUB 插图/封面落盘 `files/books/{id}/images` 与 `files/covers`、用户上传封面降采样 + EXIF 旋正、内存 LRU 位图缓存、删书清理）
  ...
  - `ui/` — Compose：`bookshelf`（书架和封面）、`stats`（统计 Tab 页）、`settings`（全局设置，含调试/日志入口）、`components`（设计系统共享组件与 LLM 面板）、`log`（日志查看器）、`navigation`（AppShell 三栏导航）、`reader`（阅读器及共享组件）、`theme`
    ...
    - `reader/ReadingTimeTracker.kt` — 阅读时长计时器（口径唯一数据源）：前台 + 2 分钟空闲判定、60 秒心跳落库、ON_STOP/退出 flush；经 sink 函数写 `ReadingTimeRepository`，`ReaderScreen` 根级 Initial 指针观察与 `updateProgress`/`navigateTo` 提供交互信号
...
- `app/src/test/java/` — LLM/SSE、迁移、DAO、设置仓库、解析器、阅读位置、分页窗口、仿真/手势、位图渲染、选词分词、词典查询、翻译状态机、阅读时长采集与伴读 JSON 层单测；`PageGeometryConsistencyTest` 是「Compose 渲染 vs 卷页位图」几何一致性的安全网（改动排版/渲染前先跑它）。共用夹具集中在 `TestFixtures.kt`（`newTextMeasurer`/`testPageStyle`/`newInMemoryDb`/`seedBookAndChapters`/`newPreferenceStore`/`inMemoryPreferenceStore`），`FakeTranslationService.kt` 独立成文件；新增测试不要再手写这些样板
- `docs/` — ADR 文档
...
- **App 外壳（书架/统计/我的 + 详情页）**：
  ...
  - 三栏导航「书架 / 统计 / 我的」由 `AppShell` 的**液态玻璃悬浮底栏**（`ui/navigation/LiquidGlassNav.kt`）承载：Tab 内容铺满整屏，每帧 record 进 `rememberGraphicsLayer`，玻璃容器用 `RenderEffect` 把这份离屏内容模糊后垫在自身下面（同一页面内容的真实背景模糊，不是组件自模糊；**仅 API 31+，26–30 降级为加深色调、不透出背景**），再叠乳白渐变色调、顶部镜面高光与 1dp 边缘高光。选中态 = 整体滑动（260ms）的浅蓝半透明胶囊 + 主题蓝图标文字；「＋」上传入口收在 AppShell，是与底栏同材质的蓝色玻璃按钮（图标色用 `onPrimary` 保对比度）。真实曲面折射无法用公开 API 稳定实现，采用近似方案，不宣称完整复刻；动画走 `animate*AsState`，随系统动画时长缩放自动降级。各页滚动容器以 `bottomChromePadding`（= 12dp 手势区距 + 64dp 条高 + 16dp 呼吸）预留滚动余量，内容可完整滚出底栏且从栏后玻璃透出。
  - **书架是刻意的例外**：遵循 ADR-006 密排视觉基线（15dp 边距、小圆角书封、`ShelfTypography` 局部字号刻度），不套 24dp 卡片体系；底栏与书架网格共用 `ShelfMetrics.PagePadding` 对齐线。
  - 统计页数据全部来自本地真实数据：聚合走 `readingStatsOf()` 纯函数（藏书/章节计数与书架共用 `getShelfItems()` 数据源、已读章节数同一算法；时长来自 `reading_time_daily` 两种聚合视图 + `ReadingTimeSnapshot`）。页面结构：书库总览 2×2 → **阅读时长卡**（今日/本周（周一起算）/累计/日均（活跃天均值）四格 + 近 7/30 天柱状图，卡内分段切换，无记录显示「暂无时长记录」空态）→ 最近阅读（行内 caption 追加「累读 X」，无记录不显示）。双语覆盖率头牌卡与书库构成卡已按用户决策删除，不要恢复；`ReadingStats` 相应字段（translatedChapters 聚合值/translationRatio/中英文/TXT/EPUB 计数）已随之移除。时长采集自 2026-09 起，此前阅读历史为空、不估算补造（底部口径说明按首条记录日期派生）。
- **阅读器是独立视觉世界（纸面 + 赭色 chrome），不随全局主题 accent 变化**：
  - chrome 表面（顶/底栏表面与文字、目录抽屉、状态面板）用 `ReaderChromeColors`（只跟阅读背景深浅走）；正文卷名（`ReadingChapterTitle` 的 section，滚动/分页/仿真位图三条路径共用）与目录/弹窗/打开过渡的强调色用 `ReaderPalette.accent`（赭色系），**禁止让卷名取 `MaterialTheme.colorScheme.primary` 或经参数另传 accent**——仿真位图 `renderPageBitmap` 只从 `palette.accent` 取色，曾因两条接线取色源不同导致翻页时卷名变色跳变。
...
- 阅读时长口径（术语见 CONTEXT.md「阅读时长」）：`ReadingTimeTracker` 在阅读器前台且最近 2 分钟内有任意交互时计时，满 60 秒心跳落库一次增量，ON_STOP/退出落盘余量，崩溃最多丢一个心跳。增量写 `reading_time_daily` 走「INSERT OR IGNORE 建行 + UPDATE 累加」两步事务，**不要改成 `ON CONFLICT ... DO UPDATE` 一句 upsert**（Robolectric 内置 SQLite 版本过旧不支持该语法，单测会挂）。交互信号 = `ReaderScreen` 根级 Initial 隧道指针观察 + `updateProgress`/`navigateTo`；弹层是独立窗口、其内部触摸不经根观察，由打开按钮的触摸与章节跳转兜底重置。只统计 App 内阅读（Web 伴读无上报通道，刻意不做）；删书随外键 CASCADE 清掉该书时长；`ReadingTimeRepository` 写失败只落 `AppLog` 不抛出。
