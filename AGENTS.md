...
- 主题色板检查（对比度是否达标 + 4 套 colorScheme 的 M3 角色是否漏填）：`python tools/check_theme_contrast.py`。**改 `ui/theme/Color.kt` 或 `Theme.kt` 后必须跑**，它会按「正文 ≥4.5:1、描边与状态图形 ≥3:1」逐对复核，并检查章节状态色的亮/暗两套在阅读器 5 档背景上都能看。漏填角色会静默落回 Material 基线的淡紫调（弹窗、抽屉、下拉菜单首当其冲）。
