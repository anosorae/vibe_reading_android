...
| **章节状态 (Chapter Status)** | `0=待翻译 1=翻译中 2=已翻译 -1=失败 3=过长`；章节翻译结果的耐久、粗粒度状态 | 翻译阶段 |
| **章节翻译任务 (Chapter Translation Task)** | 针对一本书中某一章节生成译文的工作单元，以「书籍 + 章节」唯一识别；同章重复发起互斥且幂等，不同章节可以并行。阅读焦点只决定展示哪个任务，不决定任务的开始、继续或结束 | 当前阅读章节、全局单任务（已废止） |
| **翻译阶段 (Translation Phase)** | 章节翻译任务的运行时阶段：准备、等待首 token、思考、流式输出、失败、取消；是瞬时状态，不替代章节的耐久状态 | 章节状态 |
| **翻译任务代际 (Translation Run)** | 同一章节每次实际重启翻译形成的新代际，以章节级 `translationRunId` 区分；只有当前代际可以写入该章终态，旧代际的迟到结果必须被拒绝 | 全局任务代际（已废止） |
| **已译章节数 (Translated Count)** | 一本书的 DONE 状态章节数，由 chapters 表实时派生（书架展示），不持久化缓存 | 缓存冗余列（已移除） |
| **书架 (Bookshelf)** | App 首屏与书库入口：品牌头、排序行、书籍卡片与背景插画；导入书籍、设置封面、修正原文语言、删除都从这里进入 | 阅读器 |
| **书架卡片 (Shelf Card)** | 书架上一本书的展示单元：封面、「已读/总章数」徽标、书名、已读章节数与进度条；网格与列表两种布局共用同一套字号刻度、圆角与进度条 | 列表行（另起一套样式，已否决） |
| **书架布局模式 (Shelf Layout Mode)** | 书架的两种排布：`grid` 三列密排卡片（视觉基线的主形态）与 `list` 一行一本（信息密度放宽）；默认 `grid`，按用户选择持久化 | 翻页类型 |
| **已读章节数 (Read Chapter Count)** | 卡片上「已读 N / M 章」与封面徽标里的 `N`：最后阅读章节的 1-based 序号；有阅读进度即至少 1，与 `M` 同为章节粒度（不是页数） | 已译章节数 |
| **书架进度条 (Shelf Progress Bar)** | 卡片底部的阅读进度（细轨道 + 右侧百分比）；填充带「至少看得见」的下限，未开始阅读不画填充 | 运行时页码 |
| **书架背景 (Shelf Backdrop)** | 书架页固定在内容区底部的通栏插画（翻开的书与植物）：书卡从它上面滚过，打开书架即可见，不需要滚到列表末尾；深色模式不画 | 列表末尾的页脚插画（已废止） |
| **App 底栏 (App Bottom Bar)** | 书架 / 统计 / 我的 三个 Tab 的液态玻璃悬浮胶囊导航条（Liquid Glass 风格）：真实背景模糊 + 半透明色调 + 边缘高光，选中态是滑动浅蓝胶囊；加号按钮为同材质蓝色玻璃（真实折射未实现，采用近似方案） | 阅读器底栏 |
| **统计页 (Statistics Tab)** | 「统计」Tab 的书库全景：藏书/在读/读到末章/章节总量（首卡）、阅读时长卡（今日/本周/累计/日均 + 近 7/30 天柱状图）、最近阅读（每行含该书累读）；藏书计数与时长分别由书架条目和 `reading_time_daily` 聚合，与书架同数据源、已读章节数同口径 | 双语覆盖率头牌卡、书库构成卡（已删除） |
| **阅读时长 (Reading Time)** | 按天 × 按书累计的阅读秒数（Room `reading_time_daily`，主键 bookId+epochDay，增量累加）：阅读器前台且最近 2 分钟内有任意交互时计时，每满 60 秒心跳落库一次，退出/切后台落盘余量，崩溃最多丢一个心跳；只统计 App 内阅读，Web 伴读不计入；删书随外键级联清掉该书时长；2026-09 起采集，此前历史为空、不估算补造 | 虚构/估算的历史时长 |
| **视觉基线 (Visual Baseline)** | 书架与 App 底栏的度量、取色、字号来源（设计稿实测值 + 本项目对比度门槛的裁决），见 `docs/ADR-006-bookshelf-visual-baseline.md` | 随手调色/字号 |
...
| 章节翻译任务 | 以「书籍 + 章节」识别的共享任务集合；App 与 Web 伴读观察同一组任务状态 |
...
| 书架版面度量 | `ui/bookshelf/ShelfMetrics.kt`（留白/间距/圆角/封面比例，以及列数、已读章节数、百分比、进度条填充四个纯函数） |
| 书架字号刻度 | `ui/bookshelf/ShelfTypography.kt`（局部刻度，刻意不并进 `AppTypography`） |
| 书架背景插画 | `res/drawable-nodpi/shelf_backdrop.webp`（裁自设计稿；换稿要按 ADR-006 的步骤重做） |
...
- **翻译门控**：英文书两种显示模式都翻译当前章（气泡与中文模式共用同一份中文译文），开启「提前翻译下一章」后可在当前章仍在翻译时并行预译下一章；中文书保持仅 en 模式翻译。未译回退对称：中文书 en 模式→中文原文、英文书 zh 模式→英文原文，均自动翻译自愈；英文书 en 模式下译文到达不触发重排。
...
- **书架版面只有一个度量来源**：卡片宽高、页面留白、间距、圆角、徽标内缩、进度条高度全取 `ShelfMetrics`。列数由可用宽度推（手机保底 3 列），封面高度按列宽 × 书封比例推出，**不按屏高反推**（那会让平板上的封面变成方块）。
- **书架字号是局部刻度**：改书架字号动 `ShelfTypography`，不要动 `AppTypography` —— 后者是全局共享刻度，压小会连带缩小设置页分区标题、阅读器章节标签和各类对话框。
- **「已读章节数」只有一个算法**：封面徽标与「已读 N / M 章」共用同一个派生函数，不要在卡片内各算一遍（两处不一致会直接露在界面上）。
- **书架背景是常驻的，不是列表末尾的图**：它固定在内容区底部、通栏铺满，书卡从上面滚过（ADR-006 D1b）。高度由宽度按资产比例推出并夹在 160–260dp；**不要改成 cover 铺满整屏**（1.87 宽高比的画按屏高缩放会把右侧的书和植物裁光）。它的锚点是内容区底部而非屏幕底部 —— 这一点与设计稿一致（稿子里插画带的下边界就是悬浮底栏的上沿）。
- **视觉基线稿的口径**：`docs/ADR-006` 的基线稿是图像模型的出稿，度量按「实测值反推」取；稿子自相矛盾之处（封面乱码字、行首卡片缺作者行、三列宽度不相等）按「不跟」处理，不要反过来去「修」代码。
- **配色冲突以本项目门槛为准**：稿子的强调色当正文文字只有 3.2:1，低于本项目 4.5:1 的硬约束，改色而不是改门槛（ADR-006 D2）。
...
- **章节翻译任务并发边界**：同一章节同时只有一个有效任务，重复开始视为同一任务；不同章节互不排斥，可以并行。切换阅读章节、退出阅读器或从 App/Web 的另一入口发起其他章节，只改变关注对象，不终止已有任务。重试与取消只针对指定章节；删除书籍、修正原文语言等会使整本书任务失效的操作，必须先停止该书全部任务。
- **翻译状态的耐久边界**：章节的成功、失败、待翻译等耐久终态以本地书库为准；准备、思考、流式文本等实时状态按「书籍 + 章节」区分，仅服务运行期展示。`translationRunId` 是章节级代际防护，旧代际的迟到结果不能覆盖该章当前任务。
